...
class NormTrajectory():
    ...
    def trajectory_smooth(self, position_value, x1, y1, x2, y2):
...
def norm_trajectory_array(position_array, x1, y1, x2, y2, img_width, img_height):
    x1 = np.array(x1)
    y1 = np.array(y1)
    x2 = np.array(x2)
    y2 = np.array(y2)
    ...
    # normalized
    input_norm = position_array / img_width

    # 计算box周长
    x_norm = x1 / img_width
    y_norm = y1 / img_width
    prm = (x2-x1+y2-y1)*2 / img_height
    ...
    knl_compens = [i*i for i in range(1,26)] # 卷积核
    input_norm_1 = input_norm[:, 25:]
    slide_array = np.zeros(input_norm_1.shape)
    for k in range(0, input_norm.shape[0]):
        slide_0 = []
        for i in range(25):
            slide_tmp = signal.convolve(input_norm_1[k,:i+1], knl_compens[:i+1][::-1])/sum(knl_compens[:i+1])
            slide_0.append(slide_tmp[i])
        slide_1 = signal.convolve(input_norm_1[k,:], knl_compens[::-1])/sum(knl_compens[:i+1])
        slide_1[:25] = slide_0
        slide_array[k,:] = slide_1[:-24]

    # 原曲线
    origin = input_norm[:, 25:]

    # slide与origin间积分面积，用于补偿slide间origin间差值
    lag = (origin-slide_array)*np.minimum(predict, vlo)

    # 平滑积分面积
    knl_compens = [i**2 for i in range(1,26)] # 卷积核
    lag_conv_array = np.zeros(input_norm_1.shape)
    for k in range(0, input_norm.shape[0]):
        lag_conv_0 = []
        for i in range(25):
            lag_tmp = signal.convolve(lag[k,:i+1], knl_compens[:i+1][::-1])/sum(knl_compens[:i+1])
            lag_conv_0.append(lag_tmp[i])
        lag_conv_1 = signal.convolve(lag[k], knl_compens[::-1])/sum(knl_compens)
        lag_conv_1[:25] = lag_conv_0
        lag_conv_array[k,:] = lag_conv_1[:-24]

    # 补偿后最终曲线
    compens_weight = 1 # 补偿权重
    output = (slide_array + lag_conv_array) * compens_weight

    return slide_array, output
...
norm_trajectory = NormTrajectory()

for i in range(1, 100):
    x1_l.append(x1_l[-1] + 1.1)
    y1_l.append(y1_l[-1] + 1.1)
    x2_l.append(x2_l[-1] + 1.1)
    y2_l.append(y2_l[-1] + 1.1)

    x1_norm_slide, x1_norm_output  = norm_trajectory.trajectory_smooth(x1_l, x1_l, y1_l, x2_l, y2_l)
    x1_norm_slide, x1_norm_output = x1_norm_slide * 1440, x1_norm_output * 1440
    print(x1_norm_output)
