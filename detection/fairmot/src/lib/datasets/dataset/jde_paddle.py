...
from datasets.dataset.functional import Resize, Normalize, Compose
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        transforms = [
            Resize(target_size=(self.target_width, self.target_height)), 
            Normalize(mean=(0,0,0), std=(1,1,1))
        ]

        img_path = self.files[self.count]
        img0 = cv2.imread(str(img_path))
        normalized_img, _ = Compose(transforms)(img0)
        # add an new axis in front
        img_input = normalized_img[np.newaxis, :]
        # scale_factor is calculated as: im_shape / original_im_shape
        h_scale = self.target_height / img0.shape[0]
        w_scale = self.target_width / img0.shape[1]
        img = {"im_shape": [[self.target_height, self.target_width]], "image": img_input, "scale_factor": [[h_scale, w_scale]]}
